...
// num_rendered, num_buckets, color, depth, radii, geomBuffer, binningBuffer, imgBuffer, sampleBuffer = _C.rasterize_gaussians(*args)
	...
	const bool debug)
{
	...
	return std::make_tuple(rendered, num_buckets, out_color, out_others, radii, geomBuffer, binningBuffer, imgBuffer, sampleBuffer);
}
...
RasterizeGaussiansBackwardCUDA(
	...
	const int B,					   // Acc.
	const torch::Tensor &sampleBuffer, // Acc.
	const bool debug)
{
	...
	// Scores.
	torch::Tensor scores = torch::zeros({P, 2}, means3D.options());

	if (P != 0)
	{
		CudaRasterizer::Rasterizer::backward(P, degree, M, R, B, // Acc.
											 background.contiguous().data<float>(),
											 W, H,
											 means3D.contiguous().data<float>(),
											 ...
											 reinterpret_cast<char *>(sampleBuffer.contiguous().data_ptr()), // Acc.
											 dL_dout_color.contiguous().data<float>(),
											 dL_dout_others.contiguous().data<float>(),
											 pixel_mask.contiguous().data<bool>(), // Pixel Mask.
	...
	return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dtransMat, dL_dsh, dL_dscales, dL_drotations, scores);
}
