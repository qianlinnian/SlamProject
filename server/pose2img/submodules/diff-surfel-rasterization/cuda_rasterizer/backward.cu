...
// Acc. Core Function.
// Add depth losses's backward for 2DGS.
template <uint32_t C>
__global__ void PerGaussianRenderCUDA(
	const uint2 *__restrict__ ranges,
	const uint32_t *__restrict__ point_list,
	int W, int H, int B,																																						   // Acc.
	const uint32_t *__restrict__ per_bucket_tile_offset,																														   // Acc.
	const uint32_t *__restrict__ bucket_to_tile,																																   // Acc.
	const float *__restrict__ sampled_T, const float *__restrict__ sampled_ar,																									   // Acc.
	const float *__restrict__ sampled_ar_depth, const float *__restrict__ sampled_ar_accum, const float *__restrict__ sampled_ar_normal, const float *__restrict__ sampled_ar_reg, // Acc.
	const float *__restrict__ sampled_M1, const float *__restrict__ sampled_M2,																									   // Distortion.
	float focal_x, float focal_y,
	const float *__restrict__ bg_color,
	const float2 *__restrict__ points_xy_image,
	const float4 *__restrict__ normal_opacity,
	...
	const float *__restrict__ final_Ts,
	const uint32_t *__restrict__ n_contrib,
	const uint32_t *__restrict__ max_contrib, // Acc.
	const float *__restrict__ pixel_colors,	  // Acc. 这玩意是imgState.pixel_colors
	const float *__restrict__ pixel_others,	  // Acc.
	const float *__restrict__ dL_dpixels,
	const float *__restrict__ dL_depths,
	const bool *__restrict__ pixel_mask, // Pixel Mask.
	// 很好，上面的dL_depths包含各种损失哈;
	float *__restrict__ dL_dtransMat,
	float3 *__restrict__ dL_dmean2D,
	float *__restrict__ dL_dnormal3D,
	float *__restrict__ dL_dopacity,
	...
	float *__restrict__ scores)
{
	...
	bbm = tile_id == 0 ? 0 : per_bucket_tile_offset[tile_id - 1];
	...
	// 2DGS.
	const float3 Tu = {transMats[9 * gaussian_idx + 0], transMats[9 * gaussian_idx + 1], transMats[9 * gaussian_idx + 2]};
	const float3 Tv = {transMats[9 * gaussian_idx + 3], transMats[9 * gaussian_idx + 4], transMats[9 * gaussian_idx + 5]};
	const float3 Tw = {transMats[9 * gaussian_idx + 6], transMats[9 * gaussian_idx + 7], transMats[9 * gaussian_idx + 8]};
	...
	float Register_scores[2] = {0.0f};
	// 2DGS.
	float Register_dL_dtransMat[9] = {0.0f};
	float Register_dL_dnormals3D[3] = {0.0f};
	...
	for (int i = 0; i < BLOCK_SIZE + 31; ++i)
	{
		...
		M1 = my_warp.shfl_up(M1, 1);
		M2 = my_warp.shfl_up(M2, 1);
		// D = my_warp.shfl_up(D, 1);
		...
		// 2DGS.
		// -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -
		ar_depth = my_warp.shfl_up(ar_depth, 1);
		dL_ddepth = my_warp.shfl_up(dL_ddepth, 1);

		ar_accum = my_warp.shfl_up(ar_accum, 1);
		dL_daccum = my_warp.shfl_up(dL_daccum, 1);

		for (int i = 0; i < 3; i++)
		{
			ar_normal[i] = my_warp.shfl_up(ar_normal[i], 1);
			dL_dnormal2D[i] = my_warp.shfl_up(dL_dnormal2D[i], 1);
		}
		ar_reg = my_warp.shfl_up(ar_reg, 1);
		dL_dreg = my_warp.shfl_up(dL_dreg, 1);
		// -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -

		// which pixel index should this thread deal with?
		int idx = i - my_warp.thread_rank();
		// Add pixel bias to pix.x and pix.y;
		// idx = 0~256 → 0~128
		const uint2 pix = {pix_min.x + idx % BLOCK_X, pix_min.y + idx / BLOCK_X};
		// const uint2 pix = {pix_min.x + (2*idx) % BLOCK_X, pix_min.y + (2*idx) / BLOCK_X};
		...
		if (valid_pixel && pixel_mask[pix_id] == false)
		{
			continue;
		}
		...
		if (valid_splat && valid_pixel && my_warp.thread_rank() == 0 && idx < BLOCK_SIZE)
		{
			...
			M1 = sampled_M1[global_bucket_idx * BLOCK_SIZE + idx];
			M2 = sampled_M2[global_bucket_idx * BLOCK_SIZE + idx];
			...
			// T = sampled_T[global_bucket_idx * BLOCK_SIZE + idx];
			// T = sampled_T[global_bucket_idx * BLOCK_SIZE + idx];
			ar_depth = -pixel_others[DEPTH_OFFSET * H * W + pix_id] + sampled_ar_depth[global_bucket_idx * BLOCK_SIZE + idx];
			ar_accum = -pixel_others[ALPHA_OFFSET * H * W + pix_id] + sampled_ar_accum[global_bucket_idx * BLOCK_SIZE + idx];
			for (int ch = 0; ch < 3; ++ch)
			{
				ar_normal[ch] = -pixel_others[(NORMAL_OFFSET + ch) * H * W + pix_id] + sampled_ar_normal[global_bucket_idx * BLOCK_SIZE * C + ch * BLOCK_SIZE + idx];
			}
			ar_reg = -pixel_others[DISTORTION_OFFSET * H * W + pix_id] + sampled_ar_reg[global_bucket_idx * BLOCK_SIZE + idx];
			dL_ddepth = dL_depths[DEPTH_OFFSET * H * W + pix_id];
			dL_daccum = dL_depths[ALPHA_OFFSET * H * W + pix_id];
			dL_dreg = dL_depths[DISTORTION_OFFSET * H * W + pix_id];
			for (int i = 0; i < 3; i++)
		...
		if (valid_splat && valid_pixel && 0 <= idx && idx < BLOCK_SIZE)
		{
			...
			// STEP 1 Get rho. (p, xy, pixf) → (power)
			/*
			const float2 d = { xy.x - pixf.x, xy.y - pixf.y };
			const float power = -0.5f * (con_o.x * d.x * d.x + con_o.z * d.y * d.y) - con_o.y * d.x * d.y;
			*/
			// Acc.
			// Get curr GS property.
			float3 k = pix.x * Tw - Tu;
			float3 l = pix.y * Tw - Tv;
			float3 p = cross(k, l);
			float2 s = {p.x / p.z, p.y / p.z};
			...
			float c_d = (rho3d <= rho2d) ? (s.x * Tw.x + s.y * Tw.y) + Tw.z : Tw.z; // depth
			const float m_d = far_n / (far_n - near_n) * (1 - near_n / c_d);
			const float dmd_dd = (far_n * near_n) / ((far_n - near_n) * c_d * c_d);
			...
			for (int ch = 0; ch < C; ++ch)
			{
				...
				// Score.
				Register_scores[1] += abs(dchannel_dcolor * dL_dchannel);
			}
			dL_dalpha += (-T_final / (1.0f - alpha)) * bg_dot_dpixel;
			// Score.
			Register_scores[0] += dchannel_dcolor;

			// 2DGS.
			// -  -  -  -  -  -  -  -  -  -
			float dL_dz = 0.0f;
			// 		dL_dalpha(2) | 这里少了个取地址哈, 但俺感觉不会报错；
			ar_depth += T * alpha * c_d;
			dL_dalpha += ((c_d * T) - (1.0f / (1.0f - alpha)) * (-ar_depth)) * dL_ddepth;
			//      dL_dz(1)
			dL_dz += alpha * T * dL_ddepth;
			// 		dL_dalpha(3) ar_accum就是传1做渲染;
			ar_accum += T * alpha * 1.0f;
			dL_dalpha += ((1.0f * T) - (1.0f / (1.0f - alpha)) * (-ar_accum)) * dL_daccum;

			//      dL_dalpha(4)
			for (int ch = 0; ch < 3; ++ch)
			{
				ar_normal[ch] += T * alpha * normal[ch];
				const float &dL_dchannel_normal = dL_dnormal2D[ch];
				Register_dL_dnormals3D[ch] += dchannel_dcolor * dL_dchannel_normal;
				dL_dalpha += ((normal[ch] * T) - (1.0f / (1.0f - alpha)) * (-ar_normal[ch])) * dL_dchannel_normal;
			}
			//      dL_dalpha(5)
			float A = 1 - T;
			float m = far_n / (far_n - near_n) * (1 - near_n / c_d);
			float dist = (m * m * A + M2 - 2 * m * M1);
			ar_reg += T * alpha * (dist);
			dL_dalpha += ((dist * T) - (1.0f / (1.0f - alpha)) * (-ar_reg)) * dL_dreg;
			//      dL_dz(2)
			const bool inside = pix.x < W && pix.y < H;
			const float T_final = inside ? final_Ts[pix_id] : 0;
			const float final_D = inside ? final_Ts[pix_id + H * W] : 0;
			...
			// Update T.
			T *= (1.0f - alpha);
			// After upon operations, We get (dL_dcolors, dL_dalpha, dL_dnormals3D)
			// TODO: Get dL_dz(1)(2), this is used to get dL_dmean2D.
			...
			const float dG_ddelx = -gdx * nor_o.x - gdy * nor_o.y;
			const float dG_ddely = -gdy * nor_o.z - gdx * nor_o.y;
			...
			*/
			...
			// (dL_dG, dL_dz) → (dL_dtransMat, dL_dmean2D)
			if (rho3d <= rho2d)
			{
				const float2 dL_ds = {
					dL_dG * -G * s.x + dL_dz * Tw.x,
			...
			else
			{
				const float dG_ddelx = -G * FilterInvSquare * d.x;
				const float dG_ddely = -G * FilterInvSquare * d.y;
				Register_dL_dmean2D_x += dL_dG * dG_ddelx;
				Register_dL_dmean2D_y += dL_dG * dG_ddely;
	...
	if (valid_splat)
	{
		...
		for (int ch = 0; ch < 3; ++ch)
		{
			atomicAdd(&dL_dnormal3D[gaussian_idx * C + ch], Register_dL_dnormals3D[ch]);
		}

		for (int ch = 0; ch < 9; ++ch)
		{
			atomicAdd(&dL_dtransMat[gaussian_idx * 9 + ch], Register_dL_dtransMat[ch]);
		}
		...
		atomicAdd(&(scores[gaussian_idx * 2 + 0]), Register_scores[0]);
		atomicAdd(&(scores[gaussian_idx * 2 + 1]), Register_scores[1]);
	}
}
...
void BACKWARD::render(
	...
	const float *sampled_ar_depth,
	const float *sampled_ar_accum,
	const float *sampled_ar_normal,
	const float *sampled_ar_reg,					  // Acc.
	...
	const uint32_t *max_contrib, // Acc.
	const float *pixel_colors,	 // Acc.
	const float *pixel_others,	 // Acc. (8*H*W)
	...
	float *scores)
{
	...
	PerGaussianRenderCUDA<NUM_CHAFFELS><<<((B * 32) + THREADS - 1) / THREADS, THREADS>>>(
		...
		point_list,
		W, H, B,															   // Acc.
		per_bucket_tile_offset,												   // Acc.
		bucket_to_tile,														   // Acc.
		sampled_T, sampled_ar,												   // Acc.
		sampled_ar_depth, sampled_ar_accum, sampled_ar_normal, sampled_ar_reg, // Acc.
		sampled_M1, sampled_M2,												   // Distortion.
		focal_x, focal_y,
		bg_color,
		means2D,
		normal_opacity,
