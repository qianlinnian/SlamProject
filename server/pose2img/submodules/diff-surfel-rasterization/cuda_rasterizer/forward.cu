...
#include <cub/cub.cuh>                // Acc.
#include "forward.h"
#include "auxiliary.h"
#include <cuda.h>                     // Acc.
#include "cuda_runtime.h"             // Acc.
#include "device_launch_parameters.h" // Acc.
...
renderCUDA(
	...
	// 2DGS.
	float* __restrict__ sampled_ar_depth, float* __restrict__ sampled_ar_accum, float* __restrict__ sampled_ar_normal, 
	float* __restrict__ sampled_ar_reg, 
	float* __restrict__ sampled_ar_m1, float* __restrict__ sampled_ar_m2, // Distortion Loss.
	int W, int H,
	...
	uint32_t* __restrict__ max_contrib, // Acc.
	...
	float* __restrict__ out_others)
{
	...
	// 每个tile有一个bbm, 用来索引 tile → bucket;
	// num_buckets就是当前tile下的高斯数/32;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		{	
			...
			if (j % 32 == 0) {
				...
				// 2DGS.
				// 相比于Vanilla 3DGS，这里还有过程量: 深度D, 法向量N, 深度Distort ;
				// out_others = [D, 1-T, N, median_depth, distortion], shape=(7);
				// 问题是这里面: [D, 1-T, N, distortion]是α-blending得到的，median_depth不需要α-blending;
				// [D, 1-T, N, distortion] → [sampled_ar_depth, sampled_ar_accum, sampled_ar_normal, sampled_ar_reg]
				sampled_ar_depth[(bbm * BLOCK_SIZE) + block.thread_rank()] = D;
				sampled_ar_accum[(bbm * BLOCK_SIZE) + block.thread_rank()] = 1-T;
				for (int ch = 0; ch < 3; ++ch) {
					sampled_ar_normal[(bbm * BLOCK_SIZE * CHANNELS) + ch * BLOCK_SIZE + block.thread_rank()] = N[ch];
				}
				sampled_ar_reg[(bbm * BLOCK_SIZE) + block.thread_rank()] = distortion;
				// TTD 2024/07/05
				sampled_ar_m1[(bbm * BLOCK_SIZE) + block.thread_rank()] = M1;
				sampled_ar_m2[(bbm * BLOCK_SIZE) + block.thread_rank()] = M2;
...
void FORWARD::render(
	...
	const uint32_t* per_tile_bucket_offset, uint32_t* bucket_to_tile, // Acc.
	float* sampled_T, float* sampled_ar, // Acc.
	float* sampled_ar_depth, float* sampled_ar_accum, float* sampled_ar_normal, float* sampled_ar_reg, // Acc.
	float* sampled_ar_m1, float* sampled_ar_m2, // Distortion.
	int W, int H,
	float focal_x, float focal_y,
